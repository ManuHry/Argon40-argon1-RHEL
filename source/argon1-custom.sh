...
then
	...
	echo $ID_LIKE | grep -q -E "rhel" # custom: add RHEL distro detection with following two if statements
	if [ $? -eq 0 ]
	then
		CHECKPLATFORM="Red Hat"
		CHECKPLATFORMID="$ID"
		CHECKPLATFORMVERSION=$VERSION_ID
	elif [ "$ID" = "fedora" ]
	then
		CHECKPLATFORM="Red Hat" # we want to use package manager and other adpatations like other RHEL deritatives
		CHECKPLATFORMID="$ID"
		CHECKPLATFORMVERSION=$VERSION_ID
	elif [ "$ID" = "raspbian" ]
	then
...
then
	...
	then
		...
		then
			sudo dnf config-manager --set-enabled powertools # custom: for python3-libgpiod on RHEL 8 only
			sudo dnf install -y almalinux-release-devel # custom: for python3-i2c-tools on RHEL 8 only, should work but not recommended
		elif [ "$CHECKPLATFORMID" = "rocky" ]
		then
			sudo dnf config-manager --set-enabled devel # custom: for python3-i2c-tools on RHEL 8 only, should work but not recommended
		fi
	fi
fi
...
then
	if [ "$CHECKDEVICE" = "eon" ]
	then
		pkglist=($gpiopkg $smbuspkg i2c-tools smartmontools)
	elif [ "$CHECKDEVICE" = "oneoled" ]
	then
		pkglist=($gpiopkg $smbuspkg i2c-tools python3-luma.oled)
	else
		pkglist=($gpiopkg $smbuspkg i2c-tools)
	fi
else
	# Todo handle lgpio
	...
	if [ "$CHECKDEVICE" = "eon" ]
	then
		pkglist=(wget python3 $gpiopkg $smbuspkg i2c-tools smartmontools) # custom: add wget and python3 as they are missing in minimal RHEL
	elif [ "$CHECKDEVICE" = "oneoled" ]
	then
		pkglist=(wget python3 $gpiopkg $smbuspkg i2c-tools $oledpkg) # custom: add wget and python3 as they are missing in minimal RHEL
	else
		pkglist=(wget python3 $gpiopkg $smbuspkg i2c-tools) # custom: add wget and python3 as they are missing in minimal RHEL
	fi
fi
...
for curpkg in ${pkglist[@]}; do
	if [ "$CHECKPLATFORM" = "Red Hat" ] # custom: adapt package manager to RHEL
	then
...
then
	...
	sudo wget $ARGONDOWNLOADSERVER/$imagefile -O /usr/share/pixmaps/$imagefile --quiet # custom: replace download domain by var
